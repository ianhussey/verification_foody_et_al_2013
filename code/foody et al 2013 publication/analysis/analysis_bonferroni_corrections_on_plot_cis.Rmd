...
"This yielded a final sample of 36 participants (14 males and 22 females), aged between 18 and 21 (M= 19.78 years). Participants were allocated randomly across two conditions denoted as distinction self as context (N= 18) and hierarchical self as context (N= 18)."

"M (SD)= 16.2 (6.3); hierarchical self as context: M (SD)= 18.2 (6.4)]"

No SD reported so GRIMMER can't be fit.
...
statistical_results <- read_xlsx("../../../data/extracted/foody et al 2013 publication/statistical_results.xlsx")

bind_cols(statistical_results |>
            select(test = result),
          statcheck(statistical_results, messages = FALSE)) |>
  select(test, raw, computed_p, error, decision_error) |>
...
data_processed |>
  ...
  ggplot(aes(timepoint, mean, linetype = condition, color = condition, group = condition)) +
  geom_line() +
  geom_point() +
  scale_color_viridis_d(begin = 0.3, end = 0.7) +
  theme_linedraw() +
  ...
  ylab("Mean score") +
  xlab("Time point") +
  ggtitle("Average across outcomes")
...
data_combined_baseline <- 
  bind_rows(mutate(es_assuming_ci_pooled_baseline, 
                   interval = "95% CI",
                   interval_text = "Assuming reported intervals were 95% CIs"),
            mutate(es_assuming_sem_pooled_baseline, 
                   interval = "SEM",
                   interval_text = "Assuming reported intervals were SEMs")) |>
  mutate(color = c("original", "original", "original", "new", 
                   "original", "original", "original", "new")) |>
...
data_combined_all <- 
  ...
  select(interval, interval_text, outcome_measure, color, 
...
data_change_cis <- data_baseline_postinduction |>
  ...
  mutate(mean_pooled_baseline = (mean_distinction_baseline + 
         ...
         sd_pooled_baseline = sqrt((sd_assuming_ci_distinction_baseline^2 + 
                                      sd_assuming_ci_hierarchy_baseline^2)/2),
         sd_pooled_postinduction = sqrt((sd_assuming_ci_distinction_postinduction^2 + 
                                           sd_assuming_ci_hierarchy_postinduction^2)/2),
         sd_pooled = sqrt((sd_assuming_ci_distinction_baseline^2 + 
                             sd_assuming_ci_hierarchy_baseline^2 +
                             sd_assuming_ci_distinction_postinduction^2 + 
                             sd_assuming_ci_hierarchy_postinduction^2)/4)) |>
...
data_change_sem <- data_baseline_postinduction |>
  ...
  mutate(mean_pooled_baseline = (mean_distinction_baseline + 
         ...
         mean_pooled_postinduction = (mean_distinction_postinduction + 
                                        mean_hierarchy_postinduction) / 2,
         sd_pooled_baseline = sqrt((sd_assuming_sem_distinction_baseline^2 + 
                                      sd_assuming_sem_hierarchy_baseline^2)/2),
         sd_pooled_postinduction = sqrt((sd_assuming_sem_distinction_postinduction^2 + 
                                           sd_assuming_sem_hierarchy_postinduction^2)/2),
         sd_pooled = sqrt((sd_assuming_sem_distinction_baseline^2 + 
                             sd_assuming_sem_hierarchy_baseline^2 +
                             sd_assuming_sem_distinction_postinduction^2 + 
                             sd_assuming_sem_hierarchy_postinduction^2)/4)) |>
  rename(n = n_distinction_baseline) |>
  mutate(mean_change = mean_pooled_postinduction - mean_pooled_baseline,
         cohens_d = mean_change/sd_pooled,
         hedges_g = cohens_d * ( 1 - (3 / ((4*(n + n - 2)) - 1)) )) |>
...
write_csv(data_change, "tables/change baseline to postinduction_bonferroni.csv")
