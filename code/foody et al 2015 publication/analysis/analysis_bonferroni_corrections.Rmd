...
es <- data_postintervention |>
  ...
  mutate(p_adjusted = p.adjust(p, method = "bonferroni"),
         # original: hedges_g_se = (hedges_g_ci_upper - hedges_g_ci_lower)/(1.96*2)
         # qnorm(1 - (0.0167 / 2)) returns approx 2.39, not 1.96
         z_critical = qnorm(1 - (adjusted_alpha / 2)),
         hedges_g_se = (hedges_g_ci_upper - hedges_g_ci_lower) / (z_critical * 2)) |>
...
NOTE THAT CIS ARE 98.33% CIS NOT 95% CISs, as they have bonferroni corrections applied
...
## Outcome measures (Bonferroni-corrected)

Note: the pooled "Distress" composite is deliberately excluded from the
Bonferroni-corrected analysis. Bonferroni correction adjusts for the number of
tests in a family; the pooled score is a single test (dividing alpha by 1 leaves
it unchanged), so correcting it is not meaningful. Pooling the three DVs and
Bonferroni-correcting the three DVs are alternative ways of handling the same
multiplicity. The pooled effect size and its (uncorrected) 95% CI are reported
in the no-corrections analysis.
