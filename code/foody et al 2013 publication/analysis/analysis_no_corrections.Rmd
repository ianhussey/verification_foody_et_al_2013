...
# Power implied by original study
...
Foody et al. (2013) reported *p* values for their RM-ANOVAs' interaction effects of .45, .33, .04. Using Holm corrections for multiple testing these would be:
...
es_assuming_sem <- data_postintervention |>
  ...
  mutate(p_holm_adjusted = p.adjust(p, method = "holm"),
...
es_assuming_sem |>
  select(outcome_measure,
         ...
         p_holm_adjusted) |>
  mutate(p = round_p(p),
         p_holm_adjusted = round_p(p_holm_adjusted)) |>
