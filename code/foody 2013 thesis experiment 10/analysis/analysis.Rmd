...
t_test_from_descriptives <- function(m1, m2, sd1, sd2, n1, n2, m0 = 0, equal.variance = FALSE) {
  ...
  g <- d * ( 1 - (3 / ((4*(n1 + n1 - 2)) - 1)) ) # https://stats.stackexchange.com/q/434978 by Wolfgang Vichtbauer, creator of metafor package
...
Foody (2013) experiment 10 reported *p* values for their RM-ANOVAs' interaction effects of .88, .47, .059. Using Holm corrections for multiple testing these would be:

```{r}

p.adjust(c(.88, .47, .059), method = "holm") |>
  round_p()
...
## Assuming CIs in plot are 95% CIs

### Outcome measures reported in Foody (2013) experiment 10
...
es_assuming_ci <- 
  bind_rows(
    t_test_from_descriptives(
      m1 = data_processed$mean[data_processed$outcome == "discomfort" & data_processed$condition == "distinction"],
      m2 = data_processed$mean[data_processed$outcome == "discomfort" & data_processed$condition == "hierarchy"],
      sd1 = data_processed$sd_assuming_ci[data_processed$outcome == "discomfort" & data_processed$condition == "distinction"],
      sd2 = data_processed$sd_assuming_ci[data_processed$outcome == "discomfort" & data_processed$condition == "hierarchy"],
      n1 = sample_size,
      n2 = sample_size
    ) |>
      mutate(outcome_measure = "Discomfort"),
    
    t_test_from_descriptives(
      m1 = data_processed$mean[data_processed$outcome == "anxiety" & data_processed$condition == "distinction"],
      m2 = data_processed$mean[data_processed$outcome == "anxiety" & data_processed$condition == "hierarchy"],
      sd1 = data_processed$sd_assuming_ci[data_processed$outcome == "anxiety" & data_processed$condition == "distinction"],
      sd2 = data_processed$sd_assuming_ci[data_processed$outcome == "anxiety" & data_processed$condition == "hierarchy"],
      n1 = sample_size,
      n2 = sample_size
    ) |>
      mutate(outcome_measure = "Anxiety"),
    
    t_test_from_descriptives(
      m1 = data_processed$mean[data_processed$outcome == "stress" & data_processed$condition == "distinction"],
      m2 = data_processed$mean[data_processed$outcome == "stress" & data_processed$condition == "hierarchy"],
      sd1 = data_processed$sd_assuming_ci[data_processed$outcome == "stress" & data_processed$condition == "distinction"],
      sd2 = data_processed$sd_assuming_ci[data_processed$outcome == "stress" & data_processed$condition == "hierarchy"],
      n1 = sample_size,
      n2 = sample_size
    ) |>
      mutate(outcome_measure = "Stress")
  ) |>
...
es_assuming_ci_pooled <- 
  bind_rows(
    es_assuming_ci,
    t_test_from_descriptives(
      m1 = mean(c(data_processed$mean[data_processed$outcome == "discomfort" & data_processed$condition == "distinction"],
                data_processed$mean[data_processed$outcome == "anxiety" & data_processed$condition == "distinction"],
                data_processed$mean[data_processed$outcome == "stress" & data_processed$condition == "distinction"])),
      m2 = mean(c(data_processed$mean[data_processed$outcome == "discomfort" & data_processed$condition == "hierarchy"],
                data_processed$mean[data_processed$outcome == "anxiety" & data_processed$condition == "hierarchy"],
                data_processed$mean[data_processed$outcome == "stress" & data_processed$condition == "hierarchy"])),
      sd1 = sqrt((data_processed$sd_assuming_ci[data_processed$outcome == "discomfort" & data_processed$condition == "distinction"]^2 + 
                    data_processed$sd_assuming_ci[data_processed$outcome == "anxiety" & data_processed$condition == "distinction"]^2 + 
                    data_processed$sd_assuming_ci[data_processed$outcome == "stress" & data_processed$condition == "distinction"]^2)/3),
      sd2 = sqrt((data_processed$sd_assuming_ci[data_processed$outcome == "discomfort" & data_processed$condition == "hierarchy"]^2 + 
                    data_processed$sd_assuming_ci[data_processed$outcome == "anxiety" & data_processed$condition == "hierarchy"]^2 + 
                    data_processed$sd_assuming_ci[data_processed$outcome == "stress" & data_processed$condition == "hierarchy"]^2)/3),
      n1 = sample_size,
      n2 = sample_size
      ) |>
...
## Assuming CIs in plot are SEM

### Outcome measures reported in Foody (2013) experiment 10 experiment 10
...
es_assuming_sem <- 
  bind_rows(
    t_test_from_descriptives(
      m1 = data_processed$mean[data_processed$outcome == "discomfort" & data_processed$condition == "distinction"],
      m2 = data_processed$mean[data_processed$outcome == "discomfort" & data_processed$condition == "hierarchy"],
      sd1 = data_processed$sd_assuming_sem[data_processed$outcome == "discomfort" & data_processed$condition == "distinction"],
      sd2 = data_processed$sd_assuming_sem[data_processed$outcome == "discomfort" & data_processed$condition == "hierarchy"],
      n1 = sample_size,
      n2 = sample_size
    ) |>
      mutate(outcome_measure = "Discomfort"),
    ...
    t_test_from_descriptives(
      m1 = data_processed$mean[data_processed$outcome == "anxiety" & data_processed$condition == "distinction"],
      m2 = data_processed$mean[data_processed$outcome == "anxiety" & data_processed$condition == "hierarchy"],
      sd1 = data_processed$sd_assuming_sem[data_processed$outcome == "anxiety" & data_processed$condition == "distinction"],
      sd2 = data_processed$sd_assuming_sem[data_processed$outcome == "anxiety" & data_processed$condition == "hierarchy"],
      n1 = sample_size,
      n2 = sample_size
      ) |>
      mutate(outcome_measure = "Anxiety"),
    ...
    t_test_from_descriptives(
      m1 = data_processed$mean[data_processed$outcome == "stress" & data_processed$condition == "distinction"],
      m2 = data_processed$mean[data_processed$outcome == "stress" & data_processed$condition == "hierarchy"],
      sd1 = data_processed$sd_assuming_sem[data_processed$outcome == "stress" & data_processed$condition == "distinction"],
      sd2 = data_processed$sd_assuming_sem[data_processed$outcome == "stress" & data_processed$condition == "hierarchy"],
      n1 = sample_size,
      n2 = sample_size
    ) |>
      mutate(outcome_measure = "Stress")
  ...
  bind_rows(
    ...
    t_test_from_descriptives(
      ...
      m2 = mean(c(data_processed$mean[data_processed$outcome == "discomfort" & data_processed$condition == "hierarchy"],
                data_processed$mean[data_processed$outcome == "anxiety" & data_processed$condition == "hierarchy"],
                data_processed$mean[data_processed$outcome == "stress" & data_processed$condition == "hierarchy"])),
      sd1 = sqrt((data_processed$sd_assuming_sem[data_processed$outcome == "discomfort" & data_processed$condition == "distinction"]^2 + 
                    data_processed$sd_assuming_sem[data_processed$outcome == "anxiety" & data_processed$condition == "distinction"]^2 + 
                    data_processed$sd_assuming_sem[data_processed$outcome == "stress" & data_processed$condition == "distinction"]^2)/3),
      sd2 = sqrt((data_processed$sd_assuming_sem[data_processed$outcome == "discomfort" & data_processed$condition == "hierarchy"]^2 + 
                    data_processed$sd_assuming_sem[data_processed$outcome == "anxiety" & data_processed$condition == "hierarchy"]^2 + 
                    data_processed$sd_assuming_sem[data_processed$outcome == "stress" & data_processed$condition == "hierarchy"]^2)/3),
      n1 = sample_size,
      n2 = sample_size
    ) |>
      mutate(outcome_measure = "Distress (pooled)")
