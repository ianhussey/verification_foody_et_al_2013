...
# data from Foody et al. 2013
data_processed_publication <- read_csv("../../../data/processed/foody et al 2013 publication/data_processed.csv") 
...
# data_processed |>
#   mutate(se_assuming_ci = sd_assuming_ci/sqrt(n),
#          se_assuming_sem = sd_assuming_sem/sqrt(n)) |>
#   mutate_if(is.numeric, janitor::round_half_up, digits = 2)
...
data_processed_replication <- data_processed

bind_rows(data_processed_publication,
          data_processed_replication) |> # loaded from other Rmd file
  group_by(timepoint, condition) |>
  summarize(mean = mean(mean)) |>
  ggplot(aes(timepoint, mean, linetype = fct_rev(condition), group = fct_rev(interaction(condition)))) +
  geom_line() +
...
NB differences at baseline are between d = .26 (if intervals are SEMs) to d = 0.52 (if intervals are CIs). These seem large?
...
# ggplot2::ggsave(filename = "plots/distributions bonferroni.pdf", 
#                 plot     = p_dist,
#                 device   = "pdf",
#                 width    = 5.5, 
#                 height   = 5, 
...
data_change <- 
  bind_rows(
  data_change_cis |>
    mutate(study = "Foody (2013) thesis experiment 10",
