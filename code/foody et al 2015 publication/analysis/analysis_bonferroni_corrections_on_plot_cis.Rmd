...
es_pooled <- 
  bind_rows(
    ...
    t_test_from_descriptives(
      ...
      sd1 = sqrt((data_postintervention$sd[data_postintervention$outcome == "discomfort" & data_postintervention$condition == "distinction"]^2 + 
                    data_postintervention$sd[data_postintervention$outcome == "anxiety" & data_postintervention$condition == "distinction"]^2 + 
                    data_postintervention$sd[data_postintervention$outcome == "stress" & data_postintervention$condition == "distinction"]^2)/3),
      sd2 = sqrt((data_postintervention$sd[data_postintervention$outcome == "discomfort" & data_postintervention$condition == "hierarchy"]^2 + 
                    data_postintervention$sd[data_postintervention$outcome == "anxiety" & data_postintervention$condition == "hierarchy"]^2 + 
                    data_postintervention$sd[data_postintervention$outcome == "stress" & data_postintervention$condition == "hierarchy"]^2)/3),
      n1 = sample_size,
      n2 = sample_size
    ) |>
...
data_plot <- data_processed |>
  ...
  mutate(outcome_measure = str_to_sentence(outcome)) |>
  rename(m1 = mean_distinction,
         m2 = mean_hierarchy,
         sd1 = sd_distinction,
         sd2 = sd_hierarchy)
...
p_data <- plot_dists(data_plot, "Anxiety") +
  plot_dists(data_plot, "Discomfort") +
  plot_dists(data_plot, "Stress") +
  plot_layout(ncol = 1)

p_data

# ggplot2::ggsave(filename = "plots/p_data.png",
#                 plot     = p_data,
#                 device   = "png",
#                 width    = 6,
#                 height   = 6,
...
es_pooled_baseline <- 
  bind_rows(
    ...
    t_test_from_descriptives(
      ...
      sd1 = sqrt((data_baseline$sd[data_baseline$outcome == "discomfort" & data_baseline$condition == "distinction"]^2 + 
                    data_baseline$sd[data_baseline$outcome == "anxiety" & data_baseline$condition == "distinction"]^2 + 
                    data_baseline$sd[data_baseline$outcome == "stress" & data_baseline$condition == "distinction"]^2)/3),
      sd2 = sqrt((data_baseline$sd[data_baseline$outcome == "discomfort" & data_baseline$condition == "hierarchy"]^2 + 
                    data_baseline$sd[data_baseline$outcome == "anxiety" & data_baseline$condition == "hierarchy"]^2 + 
                    data_baseline$sd[data_baseline$outcome == "stress" & data_baseline$condition == "hierarchy"]^2)/3),
      n1 = sample_size,
      n2 = sample_size
    ) |>
      mutate(outcome_measure = "Distress (pooled)")
...
data_combined_all <- 
  ...
  select(interval, outcome_measure, color, 
...
data_change <- data_baseline_postinduction |>
  pivot_wider(names_from = c(condition, timepoint), 
              values_from = c(mean, sd, n)) |>
  mutate(mean_pooled_baseline = (mean_distinction_baseline + 
                                   mean_hierarchy_baseline) / 2,
         mean_pooled_postinduction = (mean_distinction_postinduction + 
                                        mean_hierarchy_postinduction) / 2,
         sd_pooled_baseline = sqrt((sd_distinction_baseline^2 + 
                                      sd_hierarchy_baseline^2)/2),
         sd_pooled_postinduction = sqrt((sd_distinction_postinduction^2 + 
                                           sd_hierarchy_postinduction^2)/2),
         sd_pooled = sqrt((sd_distinction_baseline^2 + 
                             sd_hierarchy_baseline^2 +
                             sd_distinction_postinduction^2 + 
                             sd_hierarchy_postinduction^2)/4)) |>
  rename(n = n_distinction_baseline) |>
  mutate(mean_change = mean_pooled_postinduction - mean_pooled_baseline,
  ...
  ungroup() |>
  select(outcome, 
         mean_pooled_baseline, mean_pooled_postinduction, mean_change, 
         sd_pooled_baseline, sd_pooled_postinduction, sd_pooled, n,
...
data_change <- 
  data_change |>
  mutate(study = "Foody et al. (2015)") |>
  mutate_if(is.numeric, janitor::round_half_up, digits = 2) |>
  select(study, outcome, 
         mean_baseline = mean_pooled_baseline, mean_postinduction = mean_pooled_postinduction,
         mean_change, 
         sd_baseline = sd_pooled_baseline, 
         sd_postinduction = sd_pooled_postinduction, sd_pooled, n,
         hedges_g, lower, upper)
